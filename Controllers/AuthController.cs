...
namespace SmartPayMobileApp_Backend.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Signup([FromBody] SignupRequest request)
        {
            try
            {
                var userId = await _authService.SignupAsync(request.name, request.phoneNumber, request.email, request.password, request.cnicNumber, request.deviceId);
        ...
        public async Task<IActionResult> Login([FromBody] LoginRequest request)
        {
            try
            {
                var (isValid, userId, deviceChanged) = await _authService.ValidateUserAsync(request.email, request.password, request.deviceId);
                if (!isValid) return Unauthorized(new { message = "Invalid credentials" });

                var user = await _userRepository.GetByIdAsync(userId);
                var response = new LoginResponse { message = "Login successful", userId = userId, name = user.Name, email = user.Email, phoneNumber = user.PhoneNumber, cnicNumber = user.CnicNumber, deviceId = user.DeviceId, deviceChanged = deviceChanged, isActive = user.IsActive };
                return Ok(response);
            }
            catch (Exception ex)
            {
